...
public class Player : NetworkBehaviour {
	...
	void Start () {
        ...
        if (isLocalPlayer)
        {
            foreach(MeshRenderer mesh in playerModel)
            {
                mesh.enabled = false;
            }
        }
    ...
    {
        bar.GetComponent<Player>().chargeMeter.transform.localScale = new Vector3(1, 1, scale);
    }
    ...
    void Fire()
    {
        ...
        if(Physics.Raycast(new Ray(head.transform.position, head.transform.forward), out laserHit, 1000, rayMask))
        {
            ...
            if (laserHit.collider.CompareTag("Enemy")){
                CmdHurt(laserHit.collider.GetComponentInParent<Player>().gameObject, Mathf.CeilToInt(weaponCharge));
            }
    ...
    public void RpcSyncDamage(GameObject player, int damage)
    {
        ...
        if (other.health <= 0)
        {
            ...
            foreach(MeshRenderer m in other.playerModel)
            {
                m.enabled = false;
            }

            other.head.Find("Gun").gameObject.SetActive(false);
    ...
    {
        RpcSyncRespawn(player);
    }

    [ClientRpc]
    void RpcSyncRespawn(GameObject player)
    {
        Player playerScript = player.GetComponent<Player>();
        playerScript.myCollider.SetActive(true);
        playerScript.rb.useGravity = true;

        foreach (MeshRenderer m in playerScript.playerModel)
        {
            m.enabled = true;
        }

        playerScript.head.Find("Gun").gameObject.SetActive(true);
